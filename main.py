...
def add_transaction(user_id, text):
    ...
    if uid not in balances:
        balances[uid] = {"balance": 0, "transactions": []}
    # Add to front of list, keep only the latest 5
...
@app_commands.describe(user="The person you want to send Zeran to", amount="How much Zeran to send", reason="Why you are sending the Zeran")
async def give(interaction: discord.Interaction, user: discord.Member, amount: int, reason: str):
    ...
    # Log Transactions with Name and Reason
    add_transaction(interaction.user.id, f"Sent {amount} to {user.display_name} | Reason: {reason}")
    add_transaction(user.id, f"Received {amount} from {interaction.user.display_name} | Reason: {reason}")
...
@app_commands.describe(user="The user to pay", amount="How much Zeran to add", reason="Why you are paying them (e.g. contribution, salary)")
async def add(interaction: discord.Interaction, user: discord.Member, amount: int, reason: str):
    ...
    # Log Transaction with Name and Reason
    add_transaction(user.id, f"Admin {interaction.user.display_name} added {amount} | Reason: {reason}")
...
@app_commands.describe(user="The user to remove Zeran from", amount="How much Zeran to remove", reason="Why you are removing Zeran (e.g. tax, fine)")
async def remove(interaction: discord.Interaction, user: discord.Member, amount: int, reason: str):
    ...
    # Log Transaction with Name and Reason
    add_transaction(user.id, f"Admin {interaction.user.display_name} removed {amount} | Reason: {reason}")
...
async def zeranhelp(interaction: discord.Interaction):
    ...
    embed.add_field(name="💸 /give `<user> <amount> <reason>`", value="Send Zeran to another user.", inline=False)
    ...
    embed.add_field(name="➕ /add `<user> <amount> <reason>` 🔒", value="[Admin] Pay a user for contributions/salary.", inline=False)
    embed.add_field(name="➖ /remove `<user> <amount> <reason>` 🔒", value="[Admin] Remove Zeran from a user.", inline=False)
